...
#
# Use explicit error checks for most operations, but strict mode for build steps
set -o pipefail
...
# Generates mirror URLs for GitHub repos using common China-based proxies
_git_get_mirrors() {
    local url="$1"
    # Only mirror github.com URLs
...
_git_clone_retry() {
    ...
    log_info "Will try ${url_count} source(s) for: ${primary_url}"
...
_find_best_blob() {
    ...
    # Determine file type from requested path
    local req_basename=$(basename "$requested")
    local is_ddr=""
    local is_bl31=""
    [[ "$req_basename" == *"ddr"* ]] || [[ "$req_basename" == *"DDR"* ]] && is_ddr="1"
    [[ "$req_basename" == *"bl31"* ]] || [[ "$req_basename" == *"BL31"* ]] && is_bl31="1"
    
    # Determine file extension (default to .bin)
    local file_ext=".bin"
    [[ "$req_basename" == *.elf ]] && file_ext=".elf"
    
    local soc_path=$(dirname "$requested")
    
    # 2. Extract base pattern (remove version suffix)
    local base_pattern=$(basename "$requested" | sed 's/_v[0-9.]*\'"${file_ext}"'//' | sed 's/_eyescan//')
    
    # 3. Find all matching blobs for this SoC with correct extension, sorted by version (newest first)
    local best_match=""
    best_match=$(find "${bin_dir}/${soc_path}" -maxdepth 1 -name "${base_pattern}*${file_ext}" -print 2>/dev/null | sort -r | head -1)
    ...
    if [ -n "$is_bl31" ]; then
        # For BL31: search for bl31 .elf files
        best_match=$(find "${bin_dir}/${soc_path}" -maxdepth 1 -name "*bl31*${file_ext}" -print 2>/dev/null | sort -r | head -1)
    elif [ -n "$is_ddr" ]; then
        # For DDR: search for ddr .bin files
        best_match=$(find "${bin_dir}/${soc_path}" -maxdepth 1 -name "*ddr*.bin" -print 2>/dev/null | sort -r | head -1)
    else
        # Generic fallback
        best_match=$(find "${bin_dir}/${soc_path}" -maxdepth 1 -name "*${soc_prefix}*${file_ext}" -print 2>/dev/null | sort -r | head -1)
    fi
...
# Pre-flight: Check for required build dependencies
install_missing_pkg() {
    local pkg="$1"
    local check_cmd="$2"
    local install_pkgs="${3:-$pkg}"
    
    if ! eval "$check_cmd" 2>/dev/null; then
        log_warn "${pkg} not found - attempting to install..."
        apt-get update -qq 2>/dev/null
        if apt-get install -y -qq ${install_pkgs} 2>/dev/null; then
            log_info "${pkg} installed successfully"
        else
            log_warn "Could not install ${pkg} automatically"
            log_info "Install manually: sudo apt-get install ${install_pkgs}"
        fi
    fi
}

install_missing_pkg "python3-setuptools" "python3 -c 'import setuptools'" "python3-setuptools python3-pip"
install_missing_pkg "python3-pyelftools" "python3 -c 'import elftools'" "python3-pyelftools"
install_missing_pkg "GnuTLS dev headers" "pkg-config --exists gnutls || test -f /usr/include/gnutls/gnutls.h" "libgnutls28-dev"

# Build
log_info "Compiling U-Boot..."
if ! make -j"$(nproc)"; then
    log_error "U-Boot compilation failed"
    log_info "Common causes:"
    log_info "  - Missing python3-setuptools (for pylibfdt)"
    log_info "  - Missing build dependencies"
    log_info "  - Try: sudo apt-get install python3-setuptools"
    exit 1
fi
...
if [ -f "${UBOOT_DIR}/u-boot-rockchip.bin" ] && [ -s "${UBOOT_DIR}/u-boot-rockchip.bin" ]; then
    UBOOT_ROCKCHIP_BIN="${UBOOT_DIR}/u-boot-rockchip.bin"
fi

if [ -f "${UBOOT_DIR}/u-boot-rockchip-spi.bin" ] && [ -s "${UBOOT_DIR}/u-boot-rockchip-spi.bin" ]; then
    UBOOT_ROCKCHIP_SPI="${UBOOT_DIR}/u-boot-rockchip-spi.bin"
fi

if [ -f "${UBOOT_DIR}/idbloader.img" ] && [ -s "${UBOOT_DIR}/idbloader.img" ]; then
    IDBLOADER_IMG="${UBOOT_DIR}/idbloader.img"
fi

if [ -f "${UBOOT_DIR}/u-boot.itb" ] && [ -s "${UBOOT_DIR}/u-boot.itb" ]; then
    UBOOT_ITB="${UBOOT_DIR}/u-boot.itb"
fi

# Validate: at least one valid output must exist
if [ -z "$UBOOT_ROCKCHIP_BIN" ] && [ -z "$IDBLOADER_IMG" ] && [ -z "$UBOOT_ITB" ]; then
    log_error "U-Boot build produced no output files"
    log_error "Expected one of: u-boot-rockchip.bin, idbloader.img+u-boot.itb"
    exit 1
fi
