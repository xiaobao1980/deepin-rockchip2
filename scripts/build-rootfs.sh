...
if [[ "$SOURCE_PROFILE" == *"hwe"* ]]; then
    ...
    # Note: firmware-linux-nonfree and firmware-misc-nonfree are Debian-specific
    # and not available in Deepin repositories. linux-firmware is already included
    # in CORE_PACKAGES which covers most hardware firmware needs.
    HWE_PACKAGES=""
    if [ -n "$HWE_PACKAGES" ]; then
...
# Add keyring option (set during GPG setup above)
if [ -n "$KEYRING_OPT" ]; then
    MMDEBSTRAP_OPTS+=("$KEYRING_OPT")
fi
