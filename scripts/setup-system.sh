...
# Copy kernel debs from staging (build-kernel.sh places them there)
KERNEL_DEBS_STAGING="${WORKSPACE}/kernel-debs"
if [ -d "$KERNEL_DEBS_STAGING" ]; then
    log_info "Copying kernel debs from staging to rootfs..."
    mkdir -p "${ROOTFS_MOUNT}/boot/dtbs" "${ROOTFS_MOUNT}/boot/overlays"
    for deb in "$KERNEL_DEBS_STAGING"/linux-*.deb; do
        if [ -f "$deb" ]; then
            cp -v "$deb" "${ROOTFS_MOUNT}/boot/"
        fi
    done
    # Copy DTBs
    if [ -d "$KERNEL_DEBS_STAGING/dtbs" ]; then
        cp -v "$KERNEL_DEBS_STAGING/dtbs/"*.dtb "${ROOTFS_MOUNT}/boot/dtbs/" 2>/dev/null || true
    fi
    # Copy overlays
    if [ -d "$KERNEL_DEBS_STAGING/overlays" ]; then
        cp -v "$KERNEL_DEBS_STAGING/overlays/"*.dtbo "${ROOTFS_MOUNT}/boot/overlays/" 2>/dev/null || true
    fi
fi
...
if ls "${ROOTFS_MOUNT}/boot/"linux-*.deb 1>/dev/null 2>&1; then
    ...
    log_warn "No kernel .deb packages found in /boot"
fi
...
# Try chpasswd first, then openssl hash + direct shadow edit as fallback
_set_password() {
    local user="$1"
    local pass="$2"
    
    # Method 1: chpasswd (preferred)
    if echo "${user}:${pass}" | chpasswd 2>/dev/null; then
        echo "  Password set for ${user} via chpasswd"
        return 0
    fi
    
    # Method 2: openssl hash + direct shadow edit
    if command -v openssl >/dev/null 2>&1; then
        local hash=$(openssl passwd -6 "$pass" 2>/dev/null)
        if [ -n "$hash" ] && [ -f /etc/shadow ]; then
            local escaped_hash=$(echo "$hash" | sed 's/[\/&]/\\&/g')
            sed -i "s|^${user}:!:|${user}:${escaped_hash}:|" /etc/shadow 2>/dev/null && {
                echo "  Password set for ${user} via shadow edit"
                return 0
            }
        fi
    fi
    
    # Method 3: passwd command
    echo -e "${pass}\n${pass}" | passwd "$user" 2>/dev/null && {
        echo "  Password set for ${user} via passwd"
        return 0
    }
    
    echo "  WARNING: Could not set password for ${user}"
    echo "  Set manually after first boot: passwd ${user}"
    return 1
}
...
if [ -n "$REGULAR_USER" ]; then
    if ! id "$REGULAR_USER" &>/dev/null; then
        ...
        _set_password "${REGULAR_USER}" "${USER_PASSWD}"
...
# Note: rootfs can remain mounted during bootloader write since we write
# to the loop device (ROOT_DEVICE), not the partition
if [ -n "${UBOOT_ROCKCHIP_BIN:-}" ] && [ -f "$UBOOT_ROCKCHIP_BIN" ]; then
    ...
    log_info "Expected paths checked:"
    for f in "${WORKSPACE}/u-boot/u-boot-rockchip.bin" \
             "${WORKSPACE}/u-boot/idbloader.img" \
             "${WORKSPACE}/u-boot/u-boot.itb"; do
        if [ -f "$f" ]; then
            log_info "  Found: $f ($(stat -c%s "$f" 2>/dev/null || echo "unknown") bytes)"
        else
            log_info "  Missing: $f"
        fi
    done
...
umount -R "${ROOTFS_MOUNT}" 2>/dev/null || true
